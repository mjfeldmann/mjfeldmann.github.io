...
Basically, my point is, perfection is almost unobtainable. What we need to do is recognize that in all circumstances we are comparing apples and oranges if the environment or camera changes. So, we need to standardize as much as possible and then be honest that all of our comparisons are relative and not absolute.
